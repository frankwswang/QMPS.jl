...
using Test, Yao, MPSCircuit, Random, Statistics, Yao.ConstGate
seedNum = 1234
...
@testset "CircuitBuilder.jl + MPSC.jl" begin
    ...
    CScr = mpsCS.circuit
    CSce = mpsCS.cExtend
    CScrt = chain(2, chain(2, repeat(2, H, (2,1)), control(2, 1, 2=>Z), Measure(2, locs=2, collapseto=0)), 
                     chain(2, put(2, (2,1)=>SWAP), put(2, 1=>H), control(2, 1, 2=>Z), Measure(2, locs=2, collapseto=0)), 
    ...
    function ce_test(reg::DefaultRegister, ce::CompositeBlock, cet::CompositeBlock)
        reg3 = copy(reg) |> ce
        reg4 = copy(reg) |> cet
        @test reg3 ≈ reg4
    end
    ...
    dispatch!(Cb1, :random)
    dispatch!(Cb2, :random)
    pars1 = getiparams.(content.(collect_blocks(AbstractDiff, Cb1)))
    pars2 = getiparams.(content.(collect_blocks(AbstractDiff, Cb2)))
    ...
    DCcrt = chain(4, Cb1, swap1, Measure(4, locs=(3,4), collapseto=0), swap2, Cb2, swap1)
    DCcet = chain(6, concentrate(6, chain(4, Cb1, swap1), 3:6), concentrate(6, chain(4, Cb2, swap1), 1:4))
    ...
    r2 = copy(reg2)|> DCcet 
    @test (r1 ≈ r2) == false
    dG2 = collect_blocks(AbstractDiff, mpssDC.circuit)
    ...
    @test mpssDC.dGates == collect_blocks(AbstractDiff, DCce)
...
@testset "Diff.jl" begin
    ...
    c = markDiff(c)
    dG = collect_blocks(AbstractDiff, c)
    @test dG == [QDiff(Rx(pi)), QDiff(Ry(0.0))]

    # getQdiff(psifunc, diffblock::AbstractDiff, op::AbstractBlock)
    # getNdiff(overlapFunc::Function, dGate::AbstractDiff; δ::Real=0.01)
    c2 = DCbuilder(n,3).body
    c2 = markDiff(c2)
    dispatch!(c2, :random)
    op = put(n, 2=>Z)
    dGates = collect_blocks(AbstractDiff, c2)
